...
int find_available_slot(char *level, int *slot);
...
int find_available_slot(char *level, int *slot) {
    for (int k = 0; k < 4; k++) {
        for (int j = 0; j < 10; j++) {
            if (park.floors[k].slots[j].entry_time == 0 && park.floors[k].slots[j].subscribe == 0) {
...
void find_parked(int id, char *level, int *slot) {
    for (int k = 0; k < 4; k++) {
        for (int j = 0; j < 10; j++) {
            if (park.floors[k].slots[j].id == id) {
                *level = park.floors[k].level;
                *slot = j;
                return;
            }
...
void eval_message() {
    if (is_same(buffer, "GO", 2) ) {
        ...
        enqueue(id);
        if (park.available > 0 && find_available_slot(&tmp_lvl, &tmp_slot)) {
            temp_id = dequeue();
        ...
        int tmp_slot;
        char tmp_lvl;
        find_parked(id, &tmp_lvl, &tmp_slot);
        ...
        buf_push((id / 10) % 10 + '0', OUTBUF);
        buf_push((id % 10) + '0', OUTBUF);
        if (park.floors[lvl_no - 'A'].slots[slot_no].subscribe == 0 && park.floors[lvl_no - 'A'].slots[slot_no].entry_time == 0) {
...
void HandleInterrupt() {
    ...
    if(INTCONbits.RBIF && INTCONbits.RBIE){
        INTCONbits.RBIF = 0;
        if(PORTBbits.RB4 == 1){display_mode = !display_mode;}
    }
